...
/**
     * Constant for Pay by Bank app (Zapp) R4 custom scheme.
     */
    private static final String ZAPP_SCHEME_R4 = "zapp-r4";
	...
	/**
     * Zapp specific URI format string for R4
     */
    @SuppressWarnings("HardcodedFileSeparator")
    private static final String ZAPP_URI_FORMAT_STRING_R4 = "%s://%s?apiName=%s";
    ...
    public static boolean isCFIAppAvailable(@NonNull final Context context) {
        return isCFIAppAvailable(context, RequestType.REQUEST_TO_PAY);
    }
    ...
    public static boolean isCFIAppAvailable(@NonNull final Context context,@NonNull final RequestType requestType) {
        ...
        final String zappScheme = requestType == RequestType.REQUEST_TO_PAY ? PBBALibraryUtils.getZappScheme(context) : ZAPP_SCHEME_R4;
    ...
    public static void openBankingApp(@NonNull Context context, @NonNull final String secureToken) {
        openBankingApp(context, secureToken, RequestType.REQUEST_TO_PAY);
    }
    ...
    public static void openBankingApp(@NonNull Context context, @NonNull final String secureToken, @NonNull final RequestType requestType) {
        ...
        final Uri zappUri = Uri.parse(uriString);
    ...
    public static void showPBBAPopup(@NonNull final FragmentActivity activity, @NonNull final String secureToken, @NonNull final String brn,
        ...
        showPBBAPopup(activity, secureToken, brn, callback, timeoutTS, RequestType.REQUEST_TO_PAY);
    }
     ...
     */
    public static void showPBBAPopup(@NonNull final FragmentActivity activity, @NonNull final String secureToken, @NonNull final String brn,
                                     @NonNull final PBBAPopupCallback callback, final long timeoutTS, @NonNull final RequestType requestType) {
        ...
        final boolean hasBankApp = isCFIAppAvailable(activity, requestType);
        if (hasBankApp) {
            ...
            if (openBankAppAutomatically) {
                ...
                openBankingApp(activity, secureToken, requestType);
                return;
            } else {
                final PBBAPopupIntermediaryFragment newFragment = PBBAPopupIntermediaryFragment.newInstance(secureToken, brn, timeoutTS, requestType);
        ...
        final PBBAPopupFragment newFragment = PBBAPopupFragment.newInstance(secureToken, brn, timeoutTS, requestType);
        newFragment.setCallback(callback);
